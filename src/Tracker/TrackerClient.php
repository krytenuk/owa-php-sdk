...
#[AllowDynamicProperties] class TrackerClient extends OwaClient
{
    ...
    var ?TrackingEvent $commerce_event;

    var TrackingEvent $pageview_event;

    var array $global_event_properties = [];

    var bool $stateInit = false;

    var array $organicSearchEngines;

    // set one traffic has been attributed.
    var bool $isTrafficAttributed = false;
    ...
    public function __construct($config = null)
    {
        ...
        $this->registerStateStore('v', time() + 3600 * 24 * 365 * 10, 'assoc', true);
        $this->registerStateStore('s', time() + 3600 * 24 * 365 * 10, 'assoc', true);
        $this->registerStateStore('b', null, 'json', true);
        $cwindow = $this->getSetting('campaignAttributionWindow');
        $this->registerStateStore('c', time() + 3600 * 24 * $cwindow, 'json', true);
    ...
    private function deleteGlobalEventProperty($name): void
    {
    ...
    private function manageState(&$event): void
    {
    ...
    private function setVisitorId(&$event): void
    {
    ...
    private function setNumberPriorSessions(&$event): void
    {
    ...
    private function setFirstSessionTimestamp(&$event): void
    {
    ...
    private function setDaysSinceLastSession(&$event): void
    {
    ...
    private function setSessionId(&$event): void
    {

        $is_new_session = $this->isNewSession($event->get('timestamp'), $this->getGlobalEventProperty('last_req'));

        // if it's a new session based on time or if there is no prior session ID found, make a new session
    ...
    private function setLastRequestTime(&$event): void
    {
     ...
     * @return boolean
     */
    private function isNewSession($timestamp = '', $last_req = 0): bool
    {
     ...
     * @return boolean
     */
    public function trackEvent(TrackingEvent $event): bool
    {
        ...
        // log event to server
        return (bool) $this->logEvent($event->getEventType(), $event);
    }
     ...
     * @return boolean
     */
    public function trackStatelessEvent(TrackingEvent $event): bool
    {
     ...
     */
    public function setAllGlobalEventProperties(TrackingEvent $event): TrackingEvent
    {
    ...
    public function getAllEventProperties(TrackingEvent $event): array
    {
    ...
    public function trackPageView($event = null): bool
    {
        ...
        return (bool) $this->trackEvent($this->pageview_event);
    }

    public function trackSyndicationFeed($feed_format = 'rss', $feed_subscription_id = ''): bool
    {
    ...
    public function trackAction($action_name, $action_group = '', $action_label = '', $numeric_value = 0): bool
    {
    ...
    /**
     * Creates an ecommerce Transaction event
     *
    ...
    ): void
    {
        ...
        $this->commerce_event->set('ct_line_items', []);
     ...
     */
    public function addTransactionLineItem($order_id, $sku = '', $product_name = '', $category = '', $unit_price = 0, $quantity = 0): void
    {
     ...
     */
    public function trackTransaction(): void
    {
        ...
        if (!empty($this->commerce_event)) {
            ...
            $this->commerce_event = null;
        }
    }

    public function createSiteId($value): string
    {
...
//        }
    ...
    private function setCampaignSessionState($properties): void
    {
    ...
    function directAttributionModel($campaign_properties): void
    {
    ...
    function originalAttributionModel(&$campaign_properties): void
    {
    ...
    function setTrafficAttribution($event): void
    {
    ...
    private function inferTrafficAttribution(): void
    {
    ...
    private function isRefererSearchEngine($uri): ?array
    {
        ...
        foreach ($this->organicSearchEngines as $engine) {
            ...
            sdk::debug(sprintf('Found search engine: %s with query param %s:, query term: %s', $domain, $query_param, $term));
    ...
    function setCampaignCookie($values): void
    {
        // reset state
    ...
    // sets cookies domain
    function setCookieDomain($domain): void
    {
    ...
    public function deleteCustomVar($slot): void
    {
    ...
    private function registerStateStore($name, $expiration, $format = '', $cdh_required = ''): void
    {
        $this->state->registerStore($name, $expiration, '', $format, 'cookie', $cdh_required);
     ...
     * @param string $domain
     */
    private function setCookieDomainName(string $domain = ''): void
    {
    ...
    private function getCurrentUrl(): string
    {
        ...
        if (isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) == 'on') {
            ...
            $url .= 's';
        } elseif (isset($_SERVER['HTTP_ORIGIN']) && str_starts_with($_SERVER['HTTP_ORIGIN'], 'https')) {
            $url .= 's';
        } elseif (isset($_SERVER['HTTP_REFERER']) && str_starts_with($_SERVER['HTTP_REFERER'], 'https')) {
            $url .= 's';
        }
    ...
    private function logEvent($event_type, $event): ?ResponseInterface
    {
    ...
    private function applyNameSpaceToKeys($params): array
    {
    ...
    private function getRequestParam($name): ?string
    {
    ...
    private function makeEvent(): TrackingEvent
    {
        return new TrackingEvent;
    }

    function setSiteId(string $siteId): void
    {
        $this->siteId = $siteId;
    }
